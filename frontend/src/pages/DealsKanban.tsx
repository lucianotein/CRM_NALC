...
export default function DealsKanban() {
  ...
  if (dealsQ.isLoading) return <div className="p-6 text-slate-700">Carregando oportunidades...</div>;
  if (dealsQ.isError) return <div className="p-6 text-red-600">Erro ao carregar oportunidades</div>;
          ...
          >
            {COLUMNS.map((col) => {
              ...
              const total = items.reduce((acc, d) => {
                const n =
                  typeof d.valor_total === "number"
                    ? d.valor_total
                    : Number(String(d.valor_total || "").replace(",", "."));
                      ...
                      <div>
                        ...
                        <div className="mt-1 text-xs text-slate-500">
                          ...
                          <span className="font-semibold text-slate-700">
                            {total ? total.toLocaleString("pt-BR", { style: "currency", currency: "BRL" }) : "R$ 0,00"}
                          </span>
                        </div>
                      </div>

                      <div className="text-xs text-slate-500">
                        {/* espaço p/ futuras ações por coluna */}
                      </div>
                    </div>
                  </div>
  ...
  return (
    <div
      ...
      className="group rounded-2xl border border-slate-200 bg-white p-4 shadow-sm
                 hover:shadow-md hover:border-slate-300 transition cursor-grab active:cursor-grabbing"
    >
      <div className="flex items-start justify-between gap-2">
        <Link
          to={`/deals/${deal.id}`}
          className="text-sm font-semibold text-slate-900 leading-snug hover:underline"
